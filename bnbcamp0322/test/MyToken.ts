...
describe("MyToken Test", function () {
  ...
  async function deployMyTokenFixture() {
    ...
    const myToken = await MyToken.connect(dev).deploy("SushiToken","Sushi" );
  ...
  describe("Mint & Burn", function () {
    ...
    it("should not be able to mint a token twice", async function () {
      ...
      await myToken.connect(alice).mint(alice.address, MINT_AMOUNT);
      await expect(myToken.connect(alice).mint(alice.address, MINT_AMOUNT)).to.be.revertedWith(
        "Already minted"
      );
    });
  ...
  describe("Transfer", function () {
    it("should not be able to transfer", async function () {
      ...
      const { myToken, dev, alice } = await loadFixture(mintAliceFixture);
       ...
       await expect(
         myToken.connect(dev).transfer(alice.address, MINT_AMOUNT)
